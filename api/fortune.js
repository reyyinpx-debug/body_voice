// Vercel Serverless Function — fortune card generation (separate call from feed)
export default async function handler(req, res) {
  ...
  const { data, score, level } = req.body;
  if (!data || score === undefined || !level) {
    return res.status(400).json({ error: 'Missing data, score, or level' });
  }
